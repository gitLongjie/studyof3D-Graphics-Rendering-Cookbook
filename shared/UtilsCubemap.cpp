...
glm::vec3 faceCoordsToXYZ(int i, int j, int faceId, int faceSize) {
	...
	if (0 == faceId) return glm::vec3(-1.0f, A - 1.0f, B - 1.0f);
	if (1 == faceId) return glm::vec3(A - 1.0f, -1.0f, 1.0f - B);
	if (2 == faceId) return glm::vec3(1.0f, A - 1.0f, 1.0f - B);
	if (3 == faceId) return glm::vec3(1.0f - A, 1.0f, 1.0f - B);
	if (4 == faceId) return glm::vec3(B - 1.0f, A - 1.0f, 1.0f);
	if (5 == faceId) return glm::vec3(1.0f - B, A - 1.0f, -1.0f);
	return glm::vec3();
}

Bitmap convertEquirectangularMapToVerticalCross(const Bitmap& b)
{
	if (BitmapType::BitmapType_2D != b.getType()) {
		return Bitmap();
	}
...
Bitmap convertVerticalCrossToCubeMapFaces(const Bitmap& b)
{
	...
	cubemap.setType(BitmapType::BitmapType_Cube);;
